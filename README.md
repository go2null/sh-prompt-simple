...
It shows the short name of the current environment (distribution, OS, etc.),
the last command exit status in addition to the username and hostname.

If it finds `__git_ps1` from [git-prompt.sh](https://github.com/git/git/blob/master/contrib/completion/git-prompt.sh),
then it will use that to display the git status.  Othwerwise, it will show
the git branch when in a git checkout, and an optional clean/dirty git status
indicator.
